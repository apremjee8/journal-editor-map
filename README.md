...
A web app that plots editor-in-chief tenures for eleven journals against each editor's home institution share of papers in that journal.
...
The app listens on [http://127.0.0.1:4521](http://127.0.0.1:4521) and opens on JACC. Each journal has its own URL at `/j/<id>`. `/trends` pools sourced handovers across all eleven. Data is read from `data/bundle-parts`. The live page does not call OpenAlex.
...
- `scripts/verify-data.mjs` checks that all eleven journals have a series and sourced or gapped editors
